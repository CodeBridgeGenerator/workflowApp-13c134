...
const VerificationStep = (props) => {
    const { code, sysCode, setCode, codeError, setCodeError, onNext, resendCode, loading, setLoading } = props;
    ...
    const customInput = ({ events, props }) => {
        return (
            <input
                ...
                {...events}
                type="number"
                className={classNames('!w-12 h-16 text-4xl text-center placeholder:text-[#CED4DA] bg-white rounded-md border border-[#DEE2E6] border-solid no-spinner', codeError ? 'border-red-500' : '')}
                placeholder="0"
                onKeyDown={(e) => {
                    setCodeError(null);
                    onEnter(e);
    ...
    return (
        <div className="w-full max-w-[400px]">
            <div className="px-3 text-center">
                ...
                <p>Please enter the 6-digit code sent to your email. Check your Junk/Spam folder if doesn&apos;t arrive.</p>
            </div>
            <div className="flex flex-col items-center gap-2 mt-14">
                <InputOtp value={code} onChange={verify} length={6} inputTemplate={customInput} />
            </div>
            <div className="mt-5 text-center">
                <span className="font-semibold cursor-pointer text-primary" onClick={resendCode}>
                    Resend Code
                </span>
            </div>
            {codeError && (
            ...
            <div className="flex mt-10 justify-content-center">
                <Button label="Next" className="w-full py-3 p-button-raised p-button-rounded" onClick={onNext} disabled={!code || code.length !== 6} loading={loading}></Button>
            </div>
        </div>
    );
};
