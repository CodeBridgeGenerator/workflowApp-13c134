...
const TeamMembers = (props) => {
    const { teamMembers = [], tabs = [], ActiveTab, setActiveTab, isEdit, setIsEdit } = props;
    ...
    return (
        <div className="surface-card shadow-2 border-round p-4" style={{ position: 'relative' }}>
                ...
                <ul className="surface-card p-2 m-0 list-none flex overflow-x-auto select-none text-sm">
                    {tabs.map((tab, index) => (
                        <React.Fragment key={index}>
                            <li className="px-4">
                                <a
                                    className={classNames('p-ripple cursor-pointer px-5 py-3 flex items-center border-b-2 transition-colors duration-150', {
                                        'border-red-500 text-red-500 hover:border-blue-500': ActiveTab === index,
                            ...
                            {index < tabs.length - 1 && (
                                <li className="flex items-center">
                                    <div style={{ width: '1px', height: '50%' }} className="border-r border-gray-300"></div>
                                </li>
                            )}
                        </React.Fragment>
                    ))}
